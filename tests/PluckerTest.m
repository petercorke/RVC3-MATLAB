...
function constructor1_test(tc)
    ...
    L2 = Plucker(L);
    tc.verifyTrue( isa(L2, 'Plucker') );
    tc.verifyEqual(L2.w, [1 2 3],absTol=1e-10);
    tc.verifyEqual(L2.v, [4 5 6],absTol=1e-10);
    
    % construct from point and direction
    L = Plucker.PointDir([1 2 3], [4 5 6]);
    tc.verifyTrue(L.contains([1 2 3]));
    tc.verifyEqual(L.uw, unitvector([4 5 6]), absTol=1e-10);
end

function compact_test(tc)
    % verify double
    L = Plucker([1 2 3 4 5 6]);
    tc.verifyEqual(compact(L), [1 2 3 4 5 6], absTol=1e-10);
end
...
function constructor2_test(tc)
    ...
    L2 = Plucker(P', Q);
    tc.verifyEqual(compact(L2), compact(L), absTol=1e-10);
    L2 = Plucker(P, Q');
    tc.verifyEqual(compact(L2), compact(L), absTol=1e-10);
    L2 = Plucker(P', Q');
    tc.verifyEqual(compact(L2), compact(L), absTol=1e-10);
    ...
    tc.verifyEqual(compact(L), [w cross(w,P)], absTol=1e-10);
...
function transform_test(tc)
    P = [1 2 0]; Q = [1 2 10];  % vertical line through (1,2)
    L = Plucker(P, Q);
    % check transformation by SE3
    
    L2 = L.transform(se3());
    tc.verifyEqual(L.compact, L2.compact);
...
function plane_test(tc)
    ...
    tc.verifyEqual(compact(L), [-1 0 0 0 0 0], absTol=1e-10);
