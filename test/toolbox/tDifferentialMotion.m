classdef tDifferentialMotion < RVCTest & matlab.unittest.TestCase
    % Tests for differential motion functions
    ...
    methods (Test)
        ...
        function skew2vec(testCase)
            ...
            verifyError(testCase, @()skew2vec(1),'RVC3:skew2vec:badarg');
            verifyError(testCase, @()skew2vec(zeros(4,4)),'RVC3:skew2vec:badarg');
        ...
        function vec2skewa(testCase)
            ...
            %% 2D case
            verifyEqual(testCase, vec2skewa([3 1 2]),...
                [0 -3 1; 3 0 2; 0 0 0],absTol=1e-4);

            %% 3D case

            % test row and column vectors
            verifyEqual(testCase, vec2skewa([4 5 6 1 2 3]),...
                [0 -6 5 1; 6 0 -4 2; -5 4 0 3; 0 0 0 0],absTol=1e-4);
            verifyEqual(testCase, vec2skewa([4 5 6 1 2 3]),...
                [0 -6 5 1; 6 0 -4 2; -5 4 0 3; 0 0 0 0],absTol=1e-4);

            verifyError(testCase, @()vec2skewa(1),'RVC3:vec2skewa:badarg');
        ...
        function skewa2vec(testCase)
            %% 2D case
            verifyEqual(testCase, skewa2vec([0 -3 1; 3 0 2; 0 0 0]), [3 1 2], absTol=1e-4);

            %% 3D case
            verifyEqual(testCase, skewa2vec([0 -6 5 1; 6 0 -4 2; -5 4 0 3; 0 0 0 0]),...
                [4 5 6 1 2 3],absTol=1e-4);

            verifyError(testCase, @()skewa2vec(1),'RVC3:skewa2vec:badarg');
        ...
        function delta2tform(testCase)
            ...
            verifyEqual(testCase, delta2tform([0.4 0.5 0.6 0.1 0.2 0.3 ]),...
        ...
        function eul2jac(testCase)
            ...
            verifyError(testCase, @()eul2jac(1),'RVC3:eul2jac:badarg');
            ...
            R0 = eul2rotm(eul, "ZYZ");
            R1 = eul2rotm(eul + dth*[1 0 0], "ZYZ");
            R2 = eul2rotm(eul + dth*[0 1 0], "ZYZ");
            R3 = eul2rotm(eul + dth*[0 0 1], "ZYZ");
        ...
        function rpy2jac(testCase)
            ...
            %Testing with a scalar number input
            verifyError(testCase, @()rpy2jac(1),'RVC3:rpy2jac:badarg');
            ...
            for oo = {'XYZ', 'ZYX', 'YXZ'}
                order = oo{1};
                R0 = eul2rotm(rpy, order);
                R1 = eul2rotm(rpy + dth*[1 0 0], order);
                R2 = eul2rotm(rpy + dth*[0 1 0], order);
                R3 = eul2rotm(rpy + dth*[0 0 1], order);
        ...
        function tform2delta(testCase)
            ...
            %Testing with a scalar number input
            verifyError(testCase, @()tform2delta(1),'RVC3:tform2delta:badarg');
            verifyError(testCase, @()tform2delta(ones(3,3)),'RVC3:tform2delta:badarg');
        ...
        function tr2jac(testCase)
            ...
            verifyError(testCase, @()tr2jac(1),'RVC3:tr2jac:badarg');
        end
    end % methods
end
