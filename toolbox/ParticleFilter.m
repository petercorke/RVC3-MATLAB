...
% plotxy    display estimated vehicle path
% plotpdf   display particle distribution
%
% Properties::
...
%   Springer 2011
%
% See also stateEstimatorPF, Vehicle, RandomPath, RangeBearingSensor, PointMap, EKF.
...
classdef ParticleFilter < handle
    ...
    methods
        ...
        function run(pf, niter, varargin)
            ...
            co = colororder;
            pf.h = plot3(pf.x(:,1), pf.x(:,2), pf.x(:,3), ".", Color=co(1,:));
        ...
        function step(pf, opt)
            ...
            s = std(pf.x); %#ok<CPROPLC> 

            % std is more complex for angles, need to account for 2pi wrap
            s(3) = sqrt(sum(angdiff(repmat(wrapToPi(mn(3)).^2,size(pf.x,1),1),pf.x(:,3)))) / (pf.nparticles-1);
            pf.std = [pf.std; s];
        ...
        function plotpdf(pf)
            %ParticleFilter.plotpdf Plot particles as a PDF
            %
            % PF.plotpdf() plots a sparse PDF as a series of vertical line
            % segments of height equal to particle weight.
            clf
            hold on
            for p = 1:pf.nparticles
                xp = pf.x(p,:);
                plot3([xp(1) xp(1)], [xp(2) xp(2)], [0 pf.weight(p)], 'r');
                plot3([xp(1) xp(1)], [xp(2) xp(2)], [0 pf.weight(p)], 'k.', 'MarkerSize', 12);
        ...
        function plotxy(pf, varargin)
            %ParticleFilter.plotxy Plot vehicle position
            %
            % PF.plotxy() plots the estimated vehicle path in the xy-plane.
            %
            % PF.plotxy(LS) as above but the optional line style arguments
...
%             disp([inputname(1), ' = '])
    ...
    methods(Access=protected)
        ...
        function observe(pf, z, jf)
            ...
            z_pred(:,2) = angdiff(z_pred(:,2),repmat(z(2),size(z_pred,1),1));
