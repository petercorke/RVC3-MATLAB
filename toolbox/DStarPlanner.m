...
classdef DStarPlanner < Navigation
    ...
    methods
        ...
        % constructor
        function ds = DStarPlanner(world, varargin)
            %DStarPlanner.DStarPlanner D* constructor
            %
            % DS = DStarPlanner(MAP, OPTIONS) is a D* navigation object, and MAP is an
        ...
        function reset(ds)
            %DStarPlanner.reset Reset the planner
            %
        ...
        function s = char(ds)
            %DStarPlanner.char Convert navigation object to string
            %
            % DS.char() is a string representing the state of the DStarPlanner
            % object in human-readable form.
            %
            % See also DStarPlanner.display, Navigation.char.

            % most of the work is done by the superclass
            s = char@Navigation(ds);

            % DStarPlanner specific stuff
            if ~isempty(ds.costmap)
                s = char(s, sprintf('  costmap: %dx%d, open list %d', size(ds.costmap), size(ds.openlist,2)));
            else
        ...
        function plot(ds, varargin)
            %DStarPlanner.plot Visualize navigation environment
            %
        ...
        function plan(ds, varargin)
            %DStarPlanner.plan Plan path to goal
            %
            ...
            assert(~isempty(ds.goal), 'RTB:DStarPlanner:plan', 'no goal specified here or in constructor');
        ...
        function set_cost(ds, costmap)
            %DStarPlanner.set_cost Set the current costmap
            %
            ...
            %   calling DS.plan().
            %
            % See also DStarPlanner.modifyCost.
        ...
        function modifyCost(ds, xy, newcost)
            %DStarPlanner.modifyCost Modify cost map
            %
            % DS.modifyCost(P, C) modifies the cost map for the points described by
            ...
            %
            % See also DStarPlanner.set_cost.
            ...
            elseif size(xy,2) == numel(newcost)
    ...
    methods (Access=protected)
        ...
        function r = PROCESS_STATE(d)
            ...
            %% can we lower the path cost of any neighbors?
            if k_old == d.h(X)                          % L8
                d.message('k_old == h(X): %f\n', k_old);
                for Y=d.neighbors(X)                   % L9
        ...
        function INSERT(ds, X, h_new, where)
            ...
            elseif ds.t(X) == ds.OPEN
                k_new = min( ds.openlist(2,i), h_new ); %#ok<NASGU>
            ...
            if size(ds.openlist,2) > ds.openlist_maxlen
                ds.openlist_maxlen = size(ds.openlist,2);
            end
        ...
        % return index of neighbor states as a row vector
        function Y = neighbors(ds, X)
