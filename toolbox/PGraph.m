...
%   g.neighbors(v)             neighbors of vertex
%   g.neighbors_d(v)           neighbors of vertex and edge directions
%   g.neighbors_in(v)          neighbors with edges in
%   g.neighbors_out(v)         neighbors with edges out
...
classdef PGraph < matlab.mixin.Copyable
    ...
    methods
        ...
        function [n,c] = neighbors(g, v)
            %PGraph.neighbors Neighbours of a vertex
            %
            % N = G.neighbors(V) is a vector of ids for all vertices which are
            % directly connected neighbors of vertex V.
            %
        ...
        function [n,c] = neighbors_out(g, v)
            %PGraph.neighbors Outgoing neighbors of a vertex
            %
            % N = G.neighbors(V) is a vector of ids for all vertices which are
            % directly connected neighbors of vertex V.
            %
        ...
        function [n,c] = neighbors_in(g, v)
            %PGraph.neighbors Incoming neighbors of a vertex
            %
            % N = G.neighbors(V) is a vector of ids for all vertices which are
            % directly connected neighbors of vertex V.
            %
        ...
        function [n,c] = neighbors_d(g, v)
            %PGraph.neighbors_d Directed neighbors of a vertex
            %
            % N = G.neighbors_d(V) is a vector of ids for all vertices which are
            % directly connected neighbors of vertex V.  Elements are positive
            % if there is a link from V to the node (outgoing), and negative if the link
            % is from the node to V (incoming).
            %
            % [N,C] = G.neighbors_d(V) as above but also returns a vector C whose elements
            % are the edge costs of the paths corresponding to the vertex ids in N.
            e = g.edges(v);
        ...
        function about(g, vv)
            ...
            for v=vv
                ...
                fprintf('  neighbors: '); 
                fprintf('%d ', g.neighbors_in(v)); fprintf(' >-o-> '); 
                fprintf('%d ', g.neighbors_out(v)); fprintf('\n');
        ...
        function s = char(g)
            ...
            s = strvcat(s, sprintf('  %d edges', size(g.edgelist,2)));
            ...
            if opt.componentcolor
                ...
                for c=1:g.nc
                    ...
                    if length(vertices) == 1
                        ...
                        color = colororder(mod(c+1,size(colororder,1)-1)+1,:);
                    end
        ...
        function v = pick(g)
            ...
            [x,y] = ginput(1);
            d = vecnorm( bsxfun(@minus,[x; y], g.vertexlist(1:2,:)) );
        ...
        function [path,cost] = Astar(g, vstart, vgoal, varargin)
            ...
            while ~isempty(openSet)
                ...
                if opt.directed
                    [neighbors,costs] = g.neighbors_out(vcurrent);
                    
                else
                    [neighbors,costs] = g.neighbors(vcurrent);
                end
                ...
                for k=1:length(neighbors)
                    ...
                    if ~ismember(neighbor, openSet)
                        ...
                        openSet = union(openSet, neighbor);
                        h_score(neighbor) = g.distance(neighbor, vgoal);
