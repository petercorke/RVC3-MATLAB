%ISROT Test if value is SO(3) rotation matrix
%
% ISROT(R) is true (1) if the argument is of dimension 3x3 or 3x3xN, else false (0).
%
% ISROT(R, check=true) as above, but also checks the validity of the rotation
% matrix.
%
% Notes::
% - A valid rotation sub-matrix R has R'*R = I and det(R)=1.
% - The first form is a fast, but incomplete, test that a transform belong
%   to SO(3).
%
% See also ISHOMOG, ISROTM2, ISVEC.

% Copyright 2022-2023 Peter Corke, Witold Jachimczyk, Remo Pillat 

function h = isrot(R, options)
    arguments
        R double
        options.check (1,1) logical = false
    end

h = false;
d = size(R);

if ndims(R) >= 2
    if ~(all(d(1:2) == [3 3]))
        return %false
    end
    
    if nargin > 1
        for i = 1:size(R,3)
            RR = R(:,:,i);
            % check transpose is inverse
            e = RR'*RR - eye(3,3);
            if norm(e) > 10*eps
                return %false
            end
            % check determinant is +1
            e = abs(det(RR) - 1);
            if norm(e) > 10*eps
                return %false
            end
        end
    end
end

h = true;

end