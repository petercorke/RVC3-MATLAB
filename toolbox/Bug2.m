...
%
% The class implements the bug2 navigation algorithm.  This is a simple
% automaton that performs reactive navigation, that is, it can only sense
% the immediate presence of an obstacle.
%
% Methods::
%   Bug2        Constructor
%   run         Find a path from start to goal
...
%         start = [20,10];
%         goal = [50,35];
%         bug.run(start, goal);   % animate path
%
% Reference::
...
classdef Bug2 < Navigation
    ...
    methods
        ...
        function pp = run(bug, start, goal, varargin)
            %Bug2.run  Find a path using Bug2 reactive navigation algorithm
            %
            % B.run(START, GOAL, OPTIONS) is the path (Nx2) from START (1x2) to GOAL
        ...
        function n = next(bug, robot)
            ...
            if bug.step == 1
                ...
                if vecnorm(bug.goal - robot) == 0 % are we there yet?
                    return
                end
            ...
            if bug.step == 2
                ...
                if abs( [robot' 1]*bug.mline') <= 0.5
                    ...
                    if vecnorm(robot-bug.goal) < vecnorm(bug.H(bug.j,:)'-bug.goal)
