...
%
% An abstract superclass for implementing planar grid-based navigation
% classes.
%
% Methods::
        ...
        function nav = Navigation(varargin)
            ...
            if nargin >= 1
                if isnumeric(varargin{1}) || islogical(varargin{1})
                    % First argument is the map as numeric or logical
                    % matrix
                ...
                elseif isa(varargin{1}, "binaryOccupancyMap")
                    % Also allow object inputs as binaryOccupancyMap
                    bogMap = varargin{1};
                    nav.occgrid = flip(double(bogMap.occupancyMatrix));
                    nav.w2g = se2(eye(2), [-bogMap.GridLocationInWorld(1), ...
                        -bogMap.GridLocationInWorld(2)]);
                end
            end
...
%             nav.w2g = opt.transform;
        ...
        function pp = query(nav, start, varargin)
            ...
            while true
                ...
                % are we there yet?
                if isempty(robot)
                    path = [path nav.goal(:)]; %#ok<AGROW>
                    % yes, exit the loop
                    break
                else
                    path = [path robot(:)]; %#ok<AGROW> % append it to the path
                end
            end
        ...
        function plot_bg(nav, varargin)
            ...
            %  'colormap',@f     Specify a colormap for the distance field as a function handle, eg. @hsv
            %  'beta',B          Brighten the distance field by factor B.
            %  'labelvalues',T/F Label values in distance field (false by default)
            %  'inflated'        Show the inflated occupancy grid rather than original
            ...
            if isempty(opt.distance) || all(all(~isfinite(opt.distance)))
                ...
                d = d + mindist;   % minimum distance is cmap=2 or black
                maxdist = max(d(:));
                ...
                %  ...
                %  255 = white (maximum distance)
                %cmap = [1 0 0; opt.colormap(ceil(maxdist))];
                cmap = [1 0 0; opt.colormap(255)];
                
                % invalid distances show as black
                d = opt.distance + mindist;
                d(isnan(d)) = mindist;
                ...
                % Adjust tick labels, since we scaled color map to 255
                labels = str2double(string(cb.TickLabels));
                cb.TickLabels = num2str(labels * ...
                    (maxdist - mindist) / max(labels), 3);

                brighten(opt.beta)
            end

            % Label values, if specified            
            if opt.labelvalues
                for i = 1:size(opt.distance, 1)
                    for j = 1:size(opt.distance, 2)
                        text(gca, j, i, num2str(opt.distance(i,j), 3), ...
                            "Color", "r", "FontWeight", "bold", "FontSize", 15, "HorizontalAlignment", "center")
                    end
                end
            end
        ...
        function plot_fg(nav, varargin)
            ...
            if length(nav.goal) == 2
                ...
                if opt.start && ~isempty(nav.start)
        ...
        function disp(nav)
            %DISP Display status of navigation object
            %
            % DISP() displays the state of the navigation object in
            % human-readable form.
            %
            % Notes::
        ...
        function occ = isoccupied(nav, x, y)
            ...
            pos = round( nav.w2g.transform(pos, IsCol=true) );
