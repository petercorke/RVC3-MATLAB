...
function hout = tformplot(T, varargin)
    ...
    elseif ~ishomog(T) && ~isrot(T)
        error('SMTB:tformplot:badarg', 'tformplot operates only on transform (4x4) or rotation matrix (3x3)');
    end
...
%         end
    ...
    if opt.d_3d
        ...
        hg2 = tformplot(T, 'color', ag_color(opt.anaglyph(2)));
